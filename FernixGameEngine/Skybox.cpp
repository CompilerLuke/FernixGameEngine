...
void Skybox::Init() {
	TexturesForIBL textures = setupIBL();
	envCubemap = textures.envMap;
	irradianceCubemap = textures.irradianceMap;
}
void Skybox::Render() {
	glDepthFunc(GL_LEQUAL);  // change depth function so depth test passes when values are equal to depth buffer's content
	skybox.use();
	this->SetShaderProps(skybox);
...
void Skybox::SetIrradiance(Shader shader) {
	std::string name = "irradianceMap";
	glActiveTexture(GL_TEXTURE0);
...
TexturesForIBL setupIBL() {
	...
	float *data = stbi_loadf(assetManager.absolute("Brooklyn_Bridge_Planks_2k.hdr").c_str(), &width, &height, &nrComponents, 0);
